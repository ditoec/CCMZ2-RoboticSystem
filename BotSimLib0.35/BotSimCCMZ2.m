...
classdef BotSimCCMZ2 < handle
    ...
    properties (SetAccess = private,GetAccess = private)
        ...
        twopi; %precomputed 2*pi to speed up calculation
    end
    %public properties
    properties
        unmodifiedMap;
    ...
    methods
        %Constructor
        function bot = BotSimCCMZ2(newMap,noiseLevel,samples)
            if nargin >0
                if nargin <3
                    samples = 6;
                end
                ...
                bot.setMap(newMap);
                bot.scanConfig = generateScanConfig(bot,samples);
        ...
        function turn_move(bot,deltaAngle,distance)
            %moves the bot forward by a distance in the direction its
            %facing.  noise is added proportional to the distance moved. A
            %constant amount of angular noise is added
...
%             end
        ...
        function updateScanLines(bot,innerRad,outerRad)
            ...
            transMat = createTransMat(bot.pos)*createRotMat(bot.ang);
        ...
        function botOut = getBotPosAng(bot)
            botOut = [ bot.pos mod(bot.ang,bot.twopi) ];
        end
        ...
        function setScanConfig(bot,config)
